...
+ (id)loadObjectWithClassName:(NSString *)className andData:(id)jsonData {
    ...
    id result = [NSClassFromString(className) new];
    ...
    if (nil != mapping) {
        if ([jsonData isKindOfClass:[NSString class]]){
            result = [self loadNSStringFromClass:NSClassFromString(className)  andString:jsonData];
...
+ (id)assignObject:(id)object mapping:(ObjectMapping *)mapping dictionary:(NSDictionary *)dic {
    for (ObjectMappingEntity *entity in mapping.mappings) {
        ...
        if ([value isKindOfClass:[NSDictionary class]]) {
            id result = [[entity.typeOfClass alloc] init];
...
//加载一个不规则的JSON数组
+ (id)loadNSStringFromClass:(Class)objectClass andString:(NSString *)string {
    ...
    [self assignObject:item mapping:mapping dictionary:[NSDictionary dictionaryWithObject:string forKey:@"data"]];

...
// 星期几
static NSString *const Monday = @"周一";
static NSString *const Tuesday = @"周二";
static NSString *const Wednesday = @"周三";
static NSString *const Thursday = @"周四";
static NSString *const Friday = @"周五";
static NSString *const Saturday = @"周六";
static NSString *const Sunday = @"周日";
...
- (NSString *)yearMonthString
{
    ...
    return [NSString stringWithFormat:@"%ld",(long)info.year];
}
...
{
    TKDateInformation inf = [self dateInformation];
    return [NSString stringWithFormat:@"%02ld:%02ld",(long)inf.hour,(long)inf.minute];
}
...
- (BOOL)isPreviousMonth:(NSDate *)date
{
    ...
    if ([previousDate isSameYearAndMonth:date]) {
        return YES;
    }
...
- (BOOL)isNextMonth:(NSDate *)date
{
    ...
    if ([nextDate isSameYearAndMonth:date]) {
        return YES;
    }
